...
db.run(
  'CREATE TABLE if not exists image(imageid INTEGER PRIMARY KEY, name text NOT NULL)',
);
db.run(
  'CREATE TABLE if not exists tag(tagid INTEGER PRIMARY KEY, tag text UNIQUE NOT NULL)',
);
db.run(
  'CREATE TABLE if not exists imagetag(fk_imageid INTEGER NOT NULL, fk_tagid INTEGER NOT NULL)',
);
db.run(
  'CREATE TABLE if not exists comment(commentid INTEGER PRIMARY KEY, author TEXT, comment TEXT, fk_imageid INTEGER NOT NULL)',
);
  ...
  return new Promise((resolve, reject) => {
    db.all('SELECT * from tag', (err, rows) => {
      ...
      const newTags = tags.filter(
        (tag) => !existingTags.map((e) => e.tag).includes(tag),
      );
          ...
          db.all(
            ...
            function (err, rows) {
              ...
              const tagIds = rows
                ...
                .concat(
                  existingTags
                    .filter((et) => tags.includes(et.tag))
                    .map((e) => e.tagid),
                );
              resolve(tagIds);
            },
...
const getImagesByTags = (tags) => {
  return new Promise((res, rej) => {
    ...
    db.all(sql, [], function (err, rows) {
      ...
      const grouped = arr.map((x, i) => {
        return { imageid: i, tags: x };
      });
      const searchResult = grouped
        .filter((a) => tags.every((tag) => a.tags.includes(tag)))
        .map((x) => {
          const { name, imageid } = rows.find(
            (row) => x.imageid === row.imageid,
          );
          return { imageid, name };
        });
      res(searchResult);
